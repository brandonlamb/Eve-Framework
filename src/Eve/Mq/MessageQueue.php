...
class MessageQueue
{
	...
	public function connect($server, $port = 11211, $persistent = true, $weight = null, $timeout = 1, $retryInterval = 15, $status = true)
	{
		...
		$this->servers[$server] = array_merge(func_get_args(), array('online' => true));
	}
	...
	public function isMessageQueued($messageID)
	{
		...
		return ($head === false || $tail === false || $messageID > $tail || $messageID <= $head) ? false : true;
	}
	...
	public function queueSize()
	{
		...
		return ($tail === false || $head === false) ? 0 : $tail - $head;
	}
}
