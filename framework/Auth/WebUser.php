...
class WebUser extends Mvc\Component implements WebUserInterface
{
	...
	{
		return $this->hasState($name) ? $this->getState($name) !== null : parent::__isset($name);
	}
	...
	public function login($identity, $duration = 0)
	{
		...
		if ($this->beforeLogin($id, $states, false)) {
			...
			if ($duration > 0) {
				if ($this->allowAutoLogin === false) {
					throw new Exception(
						get_class($this) . '.allowAutoLogin must be true in order to use cookie-based authentication.'
					);
				}
	...
	{
		return (($name = $this->getState('__name')) !== null) ? $name : $this->guestName;
	}
